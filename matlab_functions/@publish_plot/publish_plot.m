classdef publish_plot < handle
    ...
    methods
        function obj = publish_plot(n_filas,n_col,varargin)
            ...
            elseif obj.data.n_filas==1 && obj.data.n_col==4
                set(gcf,'Position',[297  343  863  200])
            elseif obj.data.n_filas==1 && obj.data.n_col==6
                set(gcf,'Position',[145   627  1360   182]);
            end
        ...
        function resort_axes(obj)
            pos = get(obj.h_ax,'position');

            % Convert cell array to matrix for sorting
            data_matrix = cell2mat(pos);

            % Sort rows by the second column, then by the first column
            [~,I] = sortrows(data_matrix, [-2, 1]);
        ...
        function append_to_pdf(obj,filename,condition_to_create_new, do_save)
            ...
            filename = fullfile(filepath, [filename,'.pdf']);



            fig = obj.h_fig;

            % 1. Force MATLAB to update all layouts
            drawnow;

            % 2. Freeze figure size and axes aspect ratios
            set(fig, 'Units', 'pixels');  % or 'inches'
            pos = fig.Position;           % get current size

            % Optional: if using tiledlayout or subplot, also freeze them
            ax = findall(fig, 'Type', 'axes');
            arrayfun(@(a) set(a, 'ActivePositionProperty', 'position'), ax);

            if do_save
                if condition_to_create_new
                    exportgraphics(obj.h_fig,filename,'ContentType','vector');


                else
                    exportgraphics(obj.h_fig,filename,'ContentType','vector','Append',true);

                end

                % hack to make symbols importable in Affinity
                hack_for_affinity = false;

                if hack_for_affinity
                    % Split filename
                    [filepath, name, ext] = fileparts(filename);

                    % Temporary PDF to avoid overwriting input
                    temp_pdf = fullfile(filepath, [name '_tmp.pdf']);
                    fixed_pdf = fullfile(filepath, [name ext]);  % keep original filename

                    % Export figure to temporary PDF
                    if condition_to_create_new
                        exportgraphics(obj.h_fig, temp_pdf, 'ContentType', 'vector');
                    else
                        exportgraphics(obj.h_fig, temp_pdf, 'ContentType', 'vector', 'Append', true);
                    end

                    % Detect Ghostscript command
                    if ispc
                        gs_cmd = 'gswin64c'; % Windows
                    else
                        gs_cmd = 'gs';       % macOS/Linux
                    end

                    % Construct Ghostscript command
                    cmd = sprintf(['"%s" -dNOPAUSE -dBATCH -sDEVICE=pdfwrite ' ...
                        '-dEmbedAllFonts=true -sOutputFile="%s" "%s"'], ...
                        gs_cmd, fixed_pdf, temp_pdf);

                    % Call Ghostscript
                    [status, result] = system(cmd);

                    if status == 0
                        fprintf('Ghostscript font embedding successful: %s\n', fixed_pdf);
                        % Delete temporary file
                        delete(temp_pdf);
                    else
                        warning('Ghostscript command failed:\n%s', result);
                    end
                end
        ...
        function format(obj,varargin)
            ...
            % added Dec-20 2023
            set(obj.h_ax,'TickDir','out','TickLength',[0.015,0.015]);
        ...
        function ht = text_draw(obj, ha, strings, location)
            ...
            for i=1:length(ha)
                ...
                elseif isequal(location,'lower_right')
                    ht(i) = annotation('textbox',pos,...
                        'string',str,'horizontalalignment','right',...
                        'verticalalignment','bottom','FitBoxToText',1,'LineStyle','none');

                elseif isequal(location,'center_left')
                    ht(i) = annotation('textbox',pos,...
                        'string',str,'horizontalalignment','left',...
                        'verticalalignment','middle','FitBoxToText',1,'LineStyle','none');
        ...
        function format_text(obj, varargin)
            all_text = findall(obj.h_fig,'Type','text');
            set(all_text,varargin{:});

            for i=1:length(obj.h_ax)
                set(obj.h_ax(i),varargin{:});
            end
...
if 0 % not working in current matlab
