...
if numel(choice)>1
    ...
    if isfield(pars,'save_theta') && pars.save_theta==true
        if exist('saved_theta.mat','file')
            aux = load('saved_theta.mat','M');
            M = [aux.M; err,kappa,ndt_m,ndt_s,B0,a,d,coh0,y0a];
            save saved_theta M
        else
            M = [err,kappa,ndt_m,ndt_s,B0,a,d,coh0,y0a];
            save saved_theta M
        end
    end
    ...
    if plot_flag
        ...
        subplot(2,2,3);
        [out,~,rt_prctiles] = psychanalysis.rt_quant_plot(coh(choice==1),rt(choice==1),c(choice==1), 0);
        hold all
        colors = [0.8,0,0; 0,0.8,0]; % colors for error and correct responses
        for j=1:2
            plot(out(j).uc,out(j).rt_prc','linestyle','-','color','k','color',colors(j,:));
            hold all
            plot(out(j).uc,out(j).rt_prc','linestyle','none','marker','.','color',colors(j,:),'markersize',20);
        end
