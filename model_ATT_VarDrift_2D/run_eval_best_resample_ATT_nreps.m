...
% Simulates behavior from the ATT-VarDrift-2D model using best-fitting parameters.
%
% Runs subject-by-subject simulations with nreps repetitions, sampling
% fixation sequences from log-normal distributions fit to the data.
...
% probability of starting with left fixation (from data)
p_start_with_left = mean(data.dwells(:,1) == 1, 'omitnan');
if isnan(p_start_with_left), p_start_with_left = 0.5; end
switch_time_cost = 0;
...
parfor rep = 1:nreps
    ...
    for i = 1:numel(uni_group)
        ...
        % sample new fixation sequences for this rep
        ATT_sub = sample_attention_switches_from_pd( ...
            ntrials, T, pd, p_start_with_left, dt, switch_time_cost);

        plot_flag = 0;
        fn_fit    = @(th) wrapper_dtb_parametricbound_rt_extATT_VarDrift_2D( ...
                        th, values(I,:), ATT_sub, choice(I), [], pars, plot_flag);
        [~, P] = fn_fit(thet);
