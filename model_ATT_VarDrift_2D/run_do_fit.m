...
% Fits the ATT-VarDrift-2D model (additive intra-decisional attention with
% inter-trial value variability) to data.
%
% USAGE:
...
if dofit_flag
    ...
    parfor i = 1:size(w, 1)
        ...
        if overwrite_flag == 1 || ~exist(filename, 'file')
            ...
            plot_flag = 0;
            fn_fit    = @(theta) wrapper_dtb_parametricbound_rt_extATT_VarDrift_2D( ...
