function [err,P] = wrapper_dtb_parametricbound_rt_scale_noise(theta,rt,coh,noise_multip, choice,c, pars,plot_flag)
...
[uni,~,idx_map] = uniquetol([coh(:)+coh0,noise_multip(:)],'ByRows',1);
drift = kappa * uni(:,1);

var = 1 + uni(:,2)*noise_scaling;
min_var = 0.0001; 
var = max(var,min_var);% don't let go negative

P = dtb_fp_cc_vec(drift,t,Bup,Blo,y,y0,notabs_flag,'var',var);
P.noise_multip = uni(:,2);
P.coh = uni(:,1); 
P.idx_map = idx_map;

%% likelihood

err = logl_choiceRT_1d_var(P,choice,rt,idx_map,ndt_m,ndt_s);


%% print
fprintf('err=%.3f kappa=%.2f ndt_mu=%.2f ndt_s=%.2f B0=%.2f a=%.2f d=%.2f coh0=%.2f y0=%.2f noise scaling=%.2f \n',...
    err,kappa,ndt_m,ndt_s,B0,a,d,coh0,y0a, noise_scaling);
...
%     P.up.rt_prctiles = rt_prctiles;
    ...
    xlabel({'Response proportion','(leftward choices)'})
