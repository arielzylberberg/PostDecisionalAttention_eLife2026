function P = dtb_fp_cc_vec_dyndrifts(drift_t_matrix,t,Bup,Blo,y,y0,notabs_flag,varargin)
% Chang-Cooper method to solve 1D fp eq.
% 2015 - Ariel Zylberberg wrote it (ariel.zylberberg@gmail.com)
% 2022 - make the drifts a matrix of dims [trials x time]
...
M = chang_cooper_sparsematrix(drift_t_matrix(:,1),nd,ny,dy,dt,var);
...
for k = 2:nt
    ...
    if any(drift_t_matrix(:,k)~=drift_t_matrix(:,k-1))
        M = chang_cooper_sparsematrix(drift_t_matrix(:,k),nd,ny,dy,dt,var);
    end
