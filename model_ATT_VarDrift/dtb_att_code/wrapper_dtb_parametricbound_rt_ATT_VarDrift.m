function [err,P,ATT] = wrapper_dtb_parametricbound_rt_ATT_VarDrift(theta,rt,values,pd_fix_dur,choice,c,pars,plot_flag)
...
if std_drift==0
    ...
    bin_centers = norminv((q(1:end-1) + q(2:end)) / 2, 0, std_drift);  % size: [1, n_bins]
    % weights = diff(q);  % each bin has equal mass (e.g., 1/n_bins)

    for k=1:n_bins
        Paux = dtb_fp_cc_vec_dyndrifts(drift_t_matrix + bin_centers(k),t,Bup,Blo,y,y0,notabs_flag);
        
        if k==1
            P.up.pdf_t = Paux.up.pdf_t * 1/n_bins;
            P.lo.pdf_t = Paux.lo.pdf_t * 1/n_bins;
        else
            P.up.pdf_t = P.up.pdf_t + Paux.up.pdf_t * 1/n_bins;
            P.lo.pdf_t = P.lo.pdf_t + Paux.lo.pdf_t * 1/n_bins;
        end
...
%     err = logl_choiceRT_1d_att(P,choice,rt,values,ndt_m,ndt_s);
    ...
    %% print
    fprintf('err=%.3f kappa=%.2f ndt_mu=%.2f ndt_s=%.2f B0=%.2f a=%.2f d=%.2f coh0=%.2f y0=%.2f omega=%.2f std_drift=%.2f\n',...
        err,kappa,ndt_m,ndt_s,B0,a,d,coh0,y0a,omega, std_drift);
