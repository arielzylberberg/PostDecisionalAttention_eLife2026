function [err,P] = wrapper_dtb_parametricbound_rt_plaw_on_V(theta,rt,values,choice,c,pars,plot_flag)
...
%% scale coh
values_scaled = sign(values).*abs(values).^plaw;
coh = values_scaled(:,2)-values_scaled(:,1);
...
if ~isempty(pars) && isfield(pars,'fit_RT_only_flag')
    fit_RT_only_flag = pars.fit_RT_only_flag;
else
    fit_RT_only_flag = 0;
end
...
%%
%% print
fprintf('err=%.3f kappa=%.2f ndt_mu=%.2f ndt_s=%.2f B0=%.2f a=%.2f d=%.2f coh0=%.2f y0=%.2f plaw=%.2f gamma=%.2f \n',...
    err,kappa,ndt_m,ndt_s,B0,a,d,coh0,y0a,plaw,gamma);
