function p = fn_run_plot_best(d,m, flags)
...
p.copy_from_ax(p_aux.h_ax(3),4,1);
...
p.copy_from_ax(p_aux.h_ax(2), 6,1);
...
p.copy_from_ax(p_aux.h_ax(1), 10);
...
p.format('MarkerSize',[10,4],'FontSize',8);
% set(hl,'LineWidth',1);
...
p.xlabel({'Value difference','(left - right)'},[1,2,4]);
set(p.h_ax(3),'xtick',[0:5:20],'xticklabel',[0:5:20]);
set(p.h_ax(5),'xtick',[0:0.25:0.75]);
set(p.h_ax(6),'xtick',[-1:0.5:0],'xticklabel',[-1:0.5:0]);

set(p.h_ax(9),'xtick',[-1:1:1]);
set(p.h_ax(10),'xtick',[0:0.25:1.25]);

p.displace_ax(6,-0.035,1);

h = p.letter_the_plots('show',[1:5,7:10]);
set(h,'FontSize',11);
displace_ax(h,-0.02,1);
