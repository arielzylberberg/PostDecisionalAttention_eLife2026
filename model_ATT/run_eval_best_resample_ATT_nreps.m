...
% Simulates behavior from the ATT model using best-fitting parameters.
%
...
%
% USAGE:
%   run_eval_best_resample_ATT_nreps(1, 10)   % collapsing bounds
%   run_eval_best_resample_ATT_nreps(2, 10)   % flat bounds
%
...
parfor rep = 1:nreps
    ...
    for i = 1:numel(uni_group)
        ...
        % non-decision time (small hack for stability)
        ndt_mu    = 0.01;
        ndt_sigma = 0.01;
        theta     = [thet(1), ndt_mu, ndt_sigma, thet(2:end)];

        plot_flag = 0;
        fn_fit    = @(th) wrapper_dtb_parametricbound_rt_ATT( ...
    ...
    % last fixation at decision time
